...
이것을 기반으로 아래 규칙을 따르라:
0. 모든 출력은 한글로 작성한다. 플레이 로그는 상황을 만들기 위한 기반 자료일 뿐 해당 내용이 직접적으로 들어가면 안된다. 그리고 내용이 완전하게 중복되어선 안된다.
...
10. 반드시 유효한 JSON 형식으로 반환한다. ```json``` 표시는 붙이지 않는다. '은 사용하지 않는다.
{{
    "explain": "..."
}}
    """
...
2. 사용자의 입력 문장을 분석하여, 사용해야 할 주요 능력치(stat)를 결정한다.
   stat은 다음 중 하나여야 한다: hp, wp, str_, dex_, int_, char_
...
4. 반드시 유효한 JSON 형식으로 반환한다. ```json``` 표시는 붙이지 않는다. '은 사용하지 않는다.
{{
"action":{{
    "type": "...",
    "stat": "..."
