...
def event_after(player, event_before_log):
    ...
    event_log=  ""
    player_act = input("플레이어의 행동을 입력하세요: ")
    roll_result = get_outcome_label(roll_dice())['roll_result']
    event_log += (f'{player.name}은 {player_act} 행동을 하기로 결정하였습니다.\n')
    event_log += print_and_log(f'주사위를 굴립니다.....\n')
    event_log += print_and_log(f'주사위 결과는 {roll_result}입니다.\n')

    event_result=call_llm(build_event_prompt_after(event_before_log,player_act, roll_result))
    ...
    for stat, change_value in event_result['event']['type'].items():
        ...
        event_log += print_and_log(f'{player.name}의 능력치가 {stat}이 + {change_value} 만큼 변화했습니다.\n')
